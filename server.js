...
// ---------------- Auth de tienda ----------------
if (!STORE_KEY) {
  console.warn("[don-ramon] ⚠️  STORE_KEY no configurado — /tienda abierto (modo desarrollo). Define STORE_KEY en producción.");
}
...
db.init().then(() => {
  app.listen(PORT, () => {
    console.log(`[don-ramon] Servidor corriendo en http://localhost:${PORT}`);
    console.log(`[don-ramon] Clientes: http://localhost:${PORT}/`);
    console.log(`[don-ramon] Tienda:   http://localhost:${PORT}/tienda`);
  });
}).catch((err) => {
  console.error("[don-ramon] ❌ No se pudo inicializar la base de datos:", err.message);
  console.error("[don-ramon] Revisa DATABASE_URL (Postgres) o los permisos del archivo don-ramon-cueritos.db (SQLite).");
  process.exit(1);
});
